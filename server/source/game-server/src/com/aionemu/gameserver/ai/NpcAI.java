...
public abstract class NpcAI extends AITemplate<Npc> {
	...
	public int modifyAggroRange(int value) {
		// During post-combat alert, aggro range is expanded x1.5 so nearby players
		// stay aware that this NPC is still reactive from the previous fight.
		return getOwner().isAlerted() ? (int) (value * 1.5f) : value;
	}
