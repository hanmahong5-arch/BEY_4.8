...
public class GeoService implements GameEngine {
	...
	/**
	 * Slope-aware walkability check. Returns NaN for surfaces steeper than 45 degrees.
	 * Used by A* pathfinding to reject cliff edges as unwalkable grid nodes.
	 */
	public float getWalkableZ(int worldId, float x, float y, float zMax, float zMin, int instanceId) {
		return geoMaps.get(worldId).getZ(x, y, zMax, zMin, instanceId, true);
	}
	...
	/**
	 * Raw-coordinate collision check for A* pathfinding (no Creature required).
	 * Checks for obstacles between two world positions.
	 *
	 * @return closest collision point, or a point at/near target if path is clear
	 */
	public Vector3f getClosestCollision(float fromX, float fromY, float fromZ,
										float toX, float toY, float toZ,
										boolean atNearGroundZ, int worldId, int instanceId) {
		return geoMaps.get(worldId).getClosestCollision(fromX, fromY, fromZ, toX, toY, toZ,
			atNearGroundZ, instanceId, CollisionIntention.DEFAULT_COLLISIONS.getId(), IgnoreProperties.ANY_RACE);
	}
