...
public class NpcGameStats extends CreatureGameStats<Npc> {
	...
	public Stat2 getStat(StatEnum statEnum, Stat2 stat, CalculationType... calculationTypes) {
		...
		// Hardcore NPC tier — apply configurable multipliers via baseRate.
		// The Stat2 instance is freshly constructed by CreatureGameStats.getStat
		// (see AdditionStat in CreatureGameStats.getStat(StatEnum, float, ...)),
		// so setting baseRate here is idempotent — no compounding across ticks.
		applyHardcoreMultiplier(s, statEnum);
		return s;
	}

	private static void applyHardcoreMultiplier(Stat2 s, StatEnum statEnum) {
		double mult = switch (statEnum) {
			case MAXHP -> CustomConfig.NPC_HP_MULTIPLIER;
			case PHYSICAL_ATTACK, MAGICAL_ATTACK -> CustomConfig.NPC_ATK_MULTIPLIER;
			case SPEED -> CustomConfig.NPC_SPEED_MULTIPLIER;
			default -> 1.0;
		};
		if (mult != 1.0) {
			s.setBaseRate((float) (s.getBaseRate() * mult));
		}
	}
