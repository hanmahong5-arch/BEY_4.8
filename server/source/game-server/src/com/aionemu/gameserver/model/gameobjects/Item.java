...
public class Item extends AionObject implements Expirable, StatOwner, Persistable {
	...
	public Item(int objId, ItemTemplate itemTemplate) {
		...
		if (itemTemplate.canTune()) {
			if (CustomConfig.SKIP_IDENTIFICATION) {
				// Pre-identified: roll random stats at creation time, skip identification busywork
				this.optionalSockets = Rnd.get(0, itemTemplate.getOptionSlotBonus());
				this.enchantBonus = Rnd.get(0, itemTemplate.getMaxEnchantBonus());
				if (itemTemplate.getStatBonusSetId() != 0) {
					int bonusId = DataManager.ITEM_RANDOM_BONUSES.selectRandomBonusNumber(
						StatBonusType.INVENTORY, itemTemplate.getStatBonusSetId());
					if (bonusId > 0)
						setBonusStats(bonusId, false);
				}
				tuneCount = 0;
			} else {
	...
	public boolean isStorableInWarehouse() {
		if (CustomConfig.FREE_TRADE) return true;
