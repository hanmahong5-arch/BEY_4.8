	...
	public void setExp(long exp) {
		if (exp != this.exp || level == 0 && exp == 0) {
			...
			if (player != null) {
				...
				// Notify AI Agent of level change if level actually changed
				if (oldLevel != level)
					com.aionemu.gameserver.services.AIAgentService.getInstance().notifyLevelUp(player, oldLevel, level);
