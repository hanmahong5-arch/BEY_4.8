...
public class GatherableController extends VisibleObjectController<Gatherable> {
	...
	public void completeInteraction() {
		synchronized (this) {
			gatheringTask = null;
			// When GATHERING_ONE_CLICK is enabled, complete in a single attempt
			int target = CustomConfig.GATHERING_ONE_CLICK ? 1 : getOwner().getObjectTemplate().getHarvestCount();
			if (++gatherCount == target) {
