...
public class AttackUtil {
	...
	public static void calculateSkillResult(Effect effect, int skillDamage, DamageEffect template, boolean ignoreShield) {
		...
		if (effected instanceof Npc npcEffected) {
			damage = effected.getAi().modifyDamage(effector, damage, effect);
			// Weakness Matrix: amplify damage when the skill element matches the NPC's configured weakness.
			// Two phases: normal weakness hit (1.35x + accumulate) and active break state (1.5x bonus).
			SkillElement weakness = npcEffected.getObjectTemplate().getWeaknessElement();
			if (weakness != null && weakness != SkillElement.NONE && weakness == element) {
				long now = System.currentTimeMillis();
				if (now < npcEffected.weaknessBreakUntilMs) {
					// Break state is active — extra damage bonus while NPC defences are suppressed
					damage *= 1.5f;
				} else {
					// Standard weakness hit: amplify damage and accumulate toward the break threshold
					damage *= 1.35f;
					npcEffected.weaknessAccum += (int) damage;
					int threshold = npcEffected.getObjectTemplate().getWeaknessBreakThreshold();
					if (threshold > 0 && npcEffected.weaknessAccum >= threshold) {
						npcEffected.weaknessAccum = 0;
						npcEffected.weaknessBreakUntilMs = now + 5000L; // 5 s break window
						// Notify all nearby players with a center-screen alert
						PacketSendUtility.broadcastPacket(npcEffected,
							new SM_MESSAGE(0, null,
								"[弱点破防] " + npcEffected.getName() + " 进入破防状态！5秒内伤害大幅提升！",
