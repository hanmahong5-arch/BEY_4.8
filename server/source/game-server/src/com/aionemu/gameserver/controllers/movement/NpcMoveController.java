 ...
 *
 * <p>Ported back to Aion-Lightning upstream semantics on 2026-04-14 after
 * repeated chase / return-home / pathfinding regressions caused by the
 * custom A*, fall-stumble, and XY collision-clamp extensions. This is a
 * straight port of the upstream AL-Aion 4.x flow with BEY-specific additions
 * preserved: {@code forcedMoveToPoint}, per-tick geo-Z snap while chasing,
 * and {@code returnToLastStepOrSpawn} with canSee-based fallback.
 *
 * @author ATracer
 */
public class NpcMoveController extends CreatureMoveController<Npc> {
	...
	public void moveToDestination() {
		...
		switch (destination) {
			case TARGET_OBJECT:
				...
				if (target == null)
					return;
				// Re-read target position each tick, move linearly toward it.
				// Geo-Z snap happens inside moveToLocation; the target's own Z
				// is authoritative when the gap exceeds MOVE_CHECK_OFFSET.
	...
	private void moveToLocation(float targetX, float targetY, float targetZ) {
		...
		// Upstream-style geo-Z correction: snap to ground unless flying, path-walking,
		// or returning to spawn (returning follows stored backsteps, no geo fix).
		if (GeoDataConfig.GEO_NPC_MOVE && GeoDataConfig.GEO_ENABLE && !owner.isInFlyingState()
			&& owner.getAi().getSubState() != AISubState.WALK_PATH
			&& owner.getAi().getState() != AIState.RETURNING) {
			if (owner.getSpawn().getX() != targetDestX || owner.getSpawn().getY() != targetDestY || owner.getSpawn().getZ() != targetDestZ) {
				float geoZ = GeoService.getInstance().getZ(owner.getWorldId(), newX, newY, ownerZ + 2, ownerZ - 20, owner.getInstanceId());
				if (!Float.isNaN(geoZ)) {
