...
public class PlayerController extends CreatureController<Player> {
	...
	public void onDie(Creature lastAttacker) {
		...
		// WorldPulse: count PvP kill if killer is another Player
		if (master instanceof Player && master != player) {
			com.aionemu.gameserver.metrics.WorldPulse.getInstance().recordPvpKill();
		}
		...
		// FFA: spawn loot chest with equipped items BEFORE super.onDie() strips effects.
		// Items are detached from the player inventory, so they won't be "reset" on revive.
		// Wrapped in try/catch — a plug-in bug must NEVER crash the vanilla death path.
		if (com.aionemu.gameserver.configs.main.CustomConfig.FFA_MODE_ENABLED) {
			try {
				com.aionemu.gameserver.services.ffa.FfaModeService.getInstance().onFfaPlayerDeath(player, lastAttacker);
			} catch (Throwable t) {
				log.error("[FFA] onFfaPlayerDeath hook threw for " + player.getName(), t);
			}
		}

		// Solo fortress: award AP bounty if the victim is a fortress lord.
		// No-op for non-lord victims, NPC-only kills, or self-kills.
		if (com.aionemu.gameserver.configs.main.CustomConfig.SOLO_FORTRESS_ENABLED) {
			try {
				com.aionemu.gameserver.services.siege.SoloFortressService.getInstance().onLordKilled(player, lastAttacker);
			} catch (Throwable t) {
				log.error("[SoloFortress] onLordKilled hook threw for " + player.getName(), t);
			}
		}
		...
		// Notify AI Agent of player death
		String killerName = lastAttacker instanceof Npc ? ((Npc) lastAttacker).getObjectTemplate().getName() : master.getName();
		AIAgentService.getInstance().notifyPlayerDeath(player, lastAttacker, killerName);
	}
