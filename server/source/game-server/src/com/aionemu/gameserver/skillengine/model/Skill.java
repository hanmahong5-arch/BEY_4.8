...
public class Skill {
	...
	protected void endCast() {
		...
		// FFA hijack: if this is the configured FFA trigger skill and the caster
		// is a player, short-circuit the entire normal effect chain (no heal, no
		// item consumption, no cooldown from the skill template) and route the
		// cast completion into FfaModeService.tryToggleFromSkill. The 4-second
		// channeling animation and cast bar have already played on the client,
		// which gives us natural visual feedback for free.
		if (com.aionemu.gameserver.configs.main.CustomConfig.FFA_MODE_ENABLED
			&& com.aionemu.gameserver.configs.main.CustomConfig.FFA_TRIGGER_SKILL_ID > 0
			&& skillTemplate.getSkillId() == com.aionemu.gameserver.configs.main.CustomConfig.FFA_TRIGGER_SKILL_ID
			&& effector instanceof com.aionemu.gameserver.model.gameobjects.player.Player ffaPlayer) {
			effector.setCasting(null);
			com.aionemu.gameserver.services.ffa.FfaModeService.getInstance().tryToggleFromSkill(ffaPlayer);
			return;
		}
