...
import com.aionemu.gameserver.utils.SmartCastHelper;
		...
		switch (properties.getFirstTarget()) {
			...
			case TARGETORME:
				...
				if (skill.getFirstTarget() == null) {
					// For enemy-relation skills with no target: attempt smart-cast before
					// defaulting to self.  Covers the 218 server-side TARGETORME/ENEMY skills
					// so they participate in the same directional auto-targeting as TARGET skills.
					if (properties.getTargetRelation() == TargetRelationAttribute.ENEMY
							&& CustomConfig.SMART_CAST_ENABLED && effector instanceof Player) {
						int range = properties.getFirstTargetRange();
						Creature autoTarget = SmartCastHelper.findNearestEnemyInCone(effector, range);
						if (autoTarget != null) {
							skill.setFirstTarget(autoTarget);
							break;
						}
					}
				...
				if (skill.getFirstTarget() == null || skill.getFirstTarget().equals(effector)) {
					if (effector instanceof Player playerEffector) {
						...
						if ((relation != TargetRelationAttribute.ALL && relation != TargetRelationAttribute.MYPARTY && relation != TargetRelationAttribute.FRIEND)
							...
							// Smart Cast: when no target is selected, auto-lock onto the nearest
							// enemy in the player's facing direction cone before giving up.
							if (CustomConfig.SMART_CAST_ENABLED && skill.getFirstTarget() == null) {
								int range = properties.getFirstTargetRange();
								Creature autoTarget = SmartCastHelper.findNearestEnemyInCone(effector, range);
								if (autoTarget != null) {
									skill.setFirstTarget(autoTarget);
									break; // proceed with auto-selected target through normal TARGET validation
								}
							}
