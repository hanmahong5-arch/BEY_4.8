...
public abstract class AionServerPacket extends BaseServerPacket {
	...
	private static final Logger pktLog = LoggerFactory.getLogger("WIRE");
	...
	public final void write(AionConnection con, ByteBuffer buffer) {
		...
		buf.position(0);
		con.encrypt(b);
		// [WIRE-DEBUG] Log raw packet bytes after encryption
		if (pktLog.isInfoEnabled()) {
			byte[] raw = new byte[buf.limit()];
			buf.position(0);
			buf.get(raw);
			buf.position(0);
			StringBuilder sb = new StringBuilder();
			for (int i = 0; i < Math.min(raw.length, 32); i++) sb.append(String.format("%02X ", raw[i]));
			pktLog.info("[WIRE-DEBUG] TX {} bytes to client: {}", raw.length, sb);
		}
	}
