...
public class AionConnection extends AConnection<AionServerPacket> {
	...
	protected final boolean processData(ByteBuffer data) {
		// Temporary debug: log all incoming data
		log.info("[NET-DEBUG] processData from {} | crypt.enabled={} | remaining={} bytes | first4bytes=0x{}", getIP(), crypt.isEnabled(), data.remaining(),
			data.remaining() >= 4 ? String.format("%02X%02X%02X%02X", data.get(data.position()), data.get(data.position()+1), data.get(data.position()+2), data.get(data.position()+3)) : "N/A");
