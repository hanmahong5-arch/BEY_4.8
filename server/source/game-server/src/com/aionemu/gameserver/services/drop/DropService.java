...
public class DropService {
	...
	public void requestDropItem(Player player, int npcObjectId, int itemIndex, boolean autoLoot) {
		...
		if (requestedItem.getCount() < initialCount) {
			Player winner = requestedItem.getWinningPlayer() != null ? requestedItem.getWinningPlayer() : player;
			announceDrop(winner, template);

			// Heirloom check: only for EPIC+ (Eternal/Orange) items, avoids DB hit for common drops
			if (template.getItemQuality() != null
					&& template.getItemQuality().getQualityId() >= com.aionemu.gameserver.model.templates.item.ItemQuality.EPIC.getQualityId()) {
				com.aionemu.gameserver.model.gameobjects.Item acquiredItem =
					winner.getInventory().getFirstItemByItemId(itemId);
				VisibleObject npcObj = World.getInstance().findVisibleObject(npcObjectId);
				Npc sourceNpc = (npcObj instanceof Npc) ? (Npc) npcObj : null;
				HeirloomService.onItemDropped(winner, acquiredItem, sourceNpc);
			}
