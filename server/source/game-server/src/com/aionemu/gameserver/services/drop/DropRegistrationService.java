...
public class DropRegistrationService {
	...
	private float calculateBoostDropRate(Player killer, Npc npc) {
		...
		// Luck v2: passive drop multiplier from max FP (gear-based)
		float luckMultiplier = LuckService.getDropMultiplier(killer);
		return Rates.get(killer, RatesConfig.DROP_RATES) * boostDropRate / 100f * luckMultiplier;
	}
