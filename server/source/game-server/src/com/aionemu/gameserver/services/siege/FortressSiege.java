...
import com.aionemu.gameserver.services.ChronicleService;
import com.aionemu.gameserver.services.LegionService;
import com.aionemu.gameserver.services.NarrativeAnnouncementService;
...
public class FortressSiege extends Siege<FortressLocation> {
	...
	private void onCapture() {
		...
		// Broadcast the dethronement BEFORE clearing owner fields, so the
		// notification can still read the old ownerPlayerId / l10n name.
		// Wrapped — the dethrone broadcast is cosmetic and must not block the siege flow.
		if (getSiegeLocation().getOwnerPlayerId() != 0) {
			try {
				String newOwnerLabel = winnerRace == SiegeRace.BALAUR ? "Balaur" : winnerRace.toString();
				SoloFortressService.getInstance().onLordDethroned(getSiegeLocation(), newOwnerLabel);
			} catch (Throwable t) {
				log.error("onLordDethroned hook threw for siege " + getSiegeLocationId(), t);
			}
		}

		// Clear any previous solo ownership — any fresh capture supersedes it.
		getSiegeLocation().setOwnerPlayerId(0);
		getSiegeLocation().setOwnerCapturedAt(0);
		...
		if (SiegeRace.BALAUR == winnerRace) {
			...
			// Solo fortress ownership (单人要塞): if enabled and the top-damage
			// player qualifies, the individual becomes sole lord and the legion
			// reward path is bypassed entirely. Otherwise fall back to the
			// vanilla legion-ownership model.
			Integer topLegionId = winner.getWinnerLegionId();
			Player soloWinner = null;
			try {
				soloWinner = findSoloWinner(winner);
			} catch (Throwable t) {
				log.error("findSoloWinner threw for siege " + getSiegeLocationId(), t);
			}
			if (soloWinner != null) {
				try {
					SoloFortressService.getInstance().onSoloCapture(getSiegeLocation(), soloWinner.getObjectId(), soloWinner.getName());
				} catch (Throwable t) {
					log.error("onSoloCapture hook threw for siege " + getSiegeLocationId(), t);
				}
				getArtifact().setLegionId(0);
			} else {
		...
		// Narrative broadcast — turns the system message into a story moment
		String winnerLegionName = winnerLegion != null ? winnerLegion.getName() : null;
		NarrativeAnnouncementService.broadcastSiegeCapture(winnerRace, locL10n, winnerLegionName);

		// Persist to world chronicle
		ChronicleService.recordSiegeOutcome(winnerRace, true,
			getSiegeLocationId(), locL10n, winnerLegion);
	}

	/**
	 * Determines if the capture should go to a single player instead of a legion.
	 *
	 * <p>Picks the top-damage player from the winning race counter. Returns:
	 * <ul>
	 *   <li>the top damager, if {@code SOLO_FORTRESS_PREFER_SOLO} is true
	 *       (solo mode is the new default for any capture);</li>
	 *   <li>the top damager, if they are not in any legion (solo-only variant);</li>
	 *   <li>{@code null} otherwise — caller falls back to the legion reward chain.</li>
	 * </ul>
	 *
	 * <p>The returned player must still be online at siege-finish time, otherwise
	 * we can't look up their Player instance; in that edge case we return null
	 * and the legion path runs as a safety net.
	 */
	private Player findSoloWinner(SiegeRaceCounter winnerCounter) {
		if (!CustomConfig.SOLO_FORTRESS_ENABLED)
			return null;
		Map<Integer, Long> ordered = winnerCounter.getPlayerDamageCounter();
		if (ordered.isEmpty())
			return null;
		for (Map.Entry<Integer, Long> e : ordered.entrySet()) {
			Player p = World.getInstance().getPlayer(e.getKey());
			if (p == null)
				continue;
			if (CustomConfig.SOLO_FORTRESS_PREFER_SOLO)
				return p;
			if (p.getLegion() == null)
				return p;
			// Prefer-solo off and top is in a legion → keep searching for a solo runner-up.
		}
		return null;
	}
	...
	private void onDefended() {
		...
		// Narrative broadcast and chronicle record for successful defence
		SiegeRace defenderRace = getSiegeLocation().getRace();
		String locL10n = getSiegeLocation().getTemplate().getL10n();
		NarrativeAnnouncementService.broadcastSiegeDefend(defenderRace, locL10n);
		ChronicleService.recordSiegeOutcome(defenderRace, false,
			getSiegeLocationId(), locL10n, null);
	}
