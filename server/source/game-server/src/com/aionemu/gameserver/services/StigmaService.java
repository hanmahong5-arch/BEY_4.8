...
public class StigmaService {
	...
	public static void chargeStigma(Player player, Item stigma, Item chargeStone) {
		...
		float stigmaBaseChance = Math.max(25, 100 - (stigma.getEnchantLevel() * 10));
		final boolean isSuccess = LuckService.rollEnchant(player, stigmaBaseChance);
		...
		player.getController().addTask(TaskId.ITEM_USE, ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				if (!player.getInventory().decreaseByObjectId(chargeStone.getObjectId(), 1, ItemPacketService.ItemUpdateType.DEC_STIGMA_USE))
					return;
				if (!isSuccess) {
					if (CustomConfig.STIGMA_NO_DESTROY) {
						// Downgrade by 1 level instead of destroying (backport from 5.1)
						int newLevel = Math.max(0, stigma.getEnchantLevel() - 1);
						stigma.setEnchantLevel(newLevel);
						if (stigma.isEquipped()) {
							removeStigmaSkills(player, stigmaInfo, stigma.getEnchantLevel() + 1, false);
							addStigmaSkills(player, stigmaInfo, newLevel);
						}
