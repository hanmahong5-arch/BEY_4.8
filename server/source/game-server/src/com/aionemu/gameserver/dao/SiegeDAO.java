...
public class SiegeDAO {
	...
	public static final String SELECT_QUERY = "SELECT id, race, legion_id, occupy_count, faction_balance, owner_player_id, owner_captured_at FROM siege_locations";
	public static final String INSERT_QUERY = "INSERT INTO siege_locations (id, race, legion_id, occupy_count, faction_balance, owner_player_id, owner_captured_at) VALUES(?, ?, ?, ?, ?, ?, ?)";
	public static final String UPDATE_QUERY = "UPDATE siege_locations SET  race = ?, legion_id = ?, occupy_count = ?, faction_balance = ?, owner_player_id = ?, owner_captured_at = ? WHERE id = ?";
	...
	public static boolean loadSiegeLocations(Map<Integer, SiegeLocation> locations) {
		...
		try (Connection con = DatabaseFactory.getConnection();
			...
			while (resultSet.next()) {
				...
				loc.setOwnerPlayerId(resultSet.getInt("owner_player_id"));
				loc.setOwnerCapturedAt(resultSet.getLong("owner_captured_at"));
	...
	public static boolean updateSiegeLocation(SiegeLocation siegeLocation) {
		try (Connection con = DatabaseFactory.getConnection();
			...
			stmt.setInt(5, siegeLocation.getOwnerPlayerId());
			stmt.setLong(6, siegeLocation.getOwnerCapturedAt());
			stmt.setInt(7, siegeLocation.getLocationId());
			stmt.execute();
		} catch (Exception e) {
	...
	private static boolean insertSiegeLocation(SiegeLocation siegeLocation) {
		try (Connection con = DatabaseFactory.getConnection();
			...
			stmt.setInt(6, siegeLocation.getOwnerPlayerId());
			stmt.setLong(7, siegeLocation.getOwnerCapturedAt());
			stmt.execute();
		} catch (Exception e) {
