...
public class CustomInstanceDAO {
	...
	private static final String UPDATE_QUERY = "INSERT INTO custom_instance (player_id, rank, last_entry, max_rank, dps) VALUES (?,?,?,?,?) ON CONFLICT (player_id) DO UPDATE SET rank=EXCLUDED.rank, last_entry=EXCLUDED.last_entry, max_rank=EXCLUDED.max_rank, dps=EXCLUDED.dps";
	private static final String SELECT_TOP10_QUERY = "SELECT c.*, p.name, p.player_class FROM custom_instance c, players p WHERE c.player_id = p.id AND p.race = ? AND c.last_entry > NOW() - INTERVAL '14 days' ORDER BY c.rank DESC, c.last_entry DESC LIMIT 10";
	...
	public static List<CustomInstanceRankedPlayer> loadTop10(Race race) {
		...
		try (Connection con = DatabaseFactory.getConnection(); PreparedStatement stmt = con.prepareStatement(SELECT_TOP10_QUERY)) {
			...
			while (rset.next()) {
				...
				int maxRank = rset.getInt("max_rank");
				int dps = rset.getInt("dps");
				String name = rset.getString("name");
				PlayerClass playerClass = PlayerClass.valueOf(rset.getString("player_class"));
