...
public class PlayerAppearanceDAO {
	...
	public static boolean store(int id, PlayerAppearance pa) {
		...
		return DB.insertUpdate("INSERT INTO player_appearance ("
			...
			+ "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,"
			+ "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?"
			+ ") ON CONFLICT (player_id) DO UPDATE SET face=EXCLUDED.face, hair=EXCLUDED.hair, deco=EXCLUDED.deco, tattoo=EXCLUDED.tattoo,"
			+ " face_contour=EXCLUDED.face_contour, expression=EXCLUDED.expression, jaw_line=EXCLUDED.jaw_line,"
			+ " skin_rgb=EXCLUDED.skin_rgb, hair_rgb=EXCLUDED.hair_rgb, lip_rgb=EXCLUDED.lip_rgb, eye_rgb=EXCLUDED.eye_rgb,"
			+ " face_shape=EXCLUDED.face_shape, forehead=EXCLUDED.forehead, eye_height=EXCLUDED.eye_height,"
			+ " eye_space=EXCLUDED.eye_space, eye_width=EXCLUDED.eye_width, eye_size=EXCLUDED.eye_size,"
			+ " eye_shape=EXCLUDED.eye_shape, eye_angle=EXCLUDED.eye_angle, brow_height=EXCLUDED.brow_height,"
			+ " brow_angle=EXCLUDED.brow_angle, brow_shape=EXCLUDED.brow_shape, nose=EXCLUDED.nose,"
			+ " nose_bridge=EXCLUDED.nose_bridge, nose_width=EXCLUDED.nose_width, nose_tip=EXCLUDED.nose_tip,"
			+ " cheek=EXCLUDED.cheek, lip_height=EXCLUDED.lip_height, mouth_size=EXCLUDED.mouth_size,"
			+ " lip_size=EXCLUDED.lip_size, smile=EXCLUDED.smile, lip_shape=EXCLUDED.lip_shape,"
			+ " jaw_height=EXCLUDED.jaw_height, chin_jut=EXCLUDED.chin_jut, ear_shape=EXCLUDED.ear_shape,"
			+ " head_size=EXCLUDED.head_size, neck=EXCLUDED.neck, neck_length=EXCLUDED.neck_length,"
			+ " shoulders=EXCLUDED.shoulders, shoulder_size=EXCLUDED.shoulder_size, torso=EXCLUDED.torso,"
			+ " chest=EXCLUDED.chest, waist=EXCLUDED.waist, hips=EXCLUDED.hips,"
			+ " arm_thickness=EXCLUDED.arm_thickness, arm_length=EXCLUDED.arm_length, hand_size=EXCLUDED.hand_size,"
			+ " leg_thickness=EXCLUDED.leg_thickness, leg_length=EXCLUDED.leg_length, foot_size=EXCLUDED.foot_size,"
			+ " facial_rate=EXCLUDED.facial_rate, voice=EXCLUDED.voice, height=EXCLUDED.height", new IUStH() {
