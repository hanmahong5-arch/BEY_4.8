...
public class PlayerDAO {
	...
	public static void storePlayer(Player player) {
		try (Connection con = DatabaseFactory.getConnection();
				 PreparedStatement stmt = con.prepareStatement(
					 "UPDATE players SET name=?, exp=?, recoverexp=?, x=?, y=?, z=?, heading=?, world_id=?, gender=?, race=?, player_class=?, quest_expands=?, npc_expands=?, item_expands=?, wh_npc_expands=?, wh_bonus_expands=?, note=?, title_id=?, bonus_title_id=?, dp=?, soul_sickness=?, mailbox_letters=?, reposte_energy=?, mentor_flag_time=?, world_owner=?, luck_value=? WHERE id=?")) {
			...
			// Persist luck momentum so pity counter survives logout/restart
			stmt.setFloat(26, CustomConfig.LUCK_SYSTEM_ENABLED ? player.getLuckState().getRawLuckValue() : 0.5f);
	...
	public static int getCharacterCountOnAccount(int accountId) {
		try (Connection con = DatabaseFactory.getConnection();
				 PreparedStatement stmt = con.prepareStatement(
					 "SELECT COUNT(*) AS cnt FROM players WHERE account_id = ? AND (players.deletion_date IS NULL OR players.deletion_date > CURRENT_TIMESTAMP)")) {
	...
	public static List<PlayerAndLegionInfo> getPlayersOnInactiveAccounts(long maxExp, int daysOfAccountInactivity) {
		...
		try (Connection con = DatabaseFactory.getConnection();
				 PreparedStatement stmt = con.prepareStatement("""
					 ...
					 WHERE p.exp <= ? AND p.account_id IN (SELECT account_id FROM players GROUP BY account_id HAVING MAX(last_online) < NOW() - MAKE_INTERVAL(days => ?))
					 """)) {
	...
	/**
	 * Load the player's persisted luck momentum value from the database.
	 * Returns 0.5 (neutral baseline) if the column is absent or NULL.
	 */
	public static float loadLuckValue(int playerId) {
		try (Connection con = DatabaseFactory.getConnection();
			 PreparedStatement stmt = con.prepareStatement("SELECT luck_value FROM players WHERE id = ?")) {
			stmt.setInt(1, playerId);
			try (ResultSet rs = stmt.executeQuery()) {
				if (rs.next()) {
					float v = rs.getFloat("luck_value");
					return rs.wasNull() ? 0.5f : Math.max(0f, Math.min(1f, v));
				}
			}
		} catch (Exception e) {
			log.warn("Could not load luck_value for player id={}: {}", playerId, e.getMessage());
		}
		return 0.5f;
	}
}
