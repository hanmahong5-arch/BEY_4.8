...
public class AbyssRankDAO {
	...
	private static final String SELECT_RANKING_LIST_PLAYERS = "SELECT a.rank_pos, a.old_rank_pos, p.id, p.name, p.race, p.exp, a.rank, a.ap, a.gp, p.title_id, p.player_class, p.gender, l.name AS legion_name FROM abyss_rank a JOIN players p ON a.player_id = p.id LEFT JOIN legion_members lm ON lm.player_id = p.id LEFT JOIN legions l ON l.id = lm.legion_id WHERE a.rank_pos > 0";
	...
	private static final String RESET_RANKING_LIST_PLAYERS = "UPDATE abyss_rank SET old_rank_pos = rank_pos, rank_pos = 0 WHERE rank_pos > 0";
	private static final String RESET_RANKING_LIST_LEGIONS = "UPDATE legions SET old_rank_pos = rank_pos, rank_pos = 0 WHERE rank_pos > 0";
	// Player ranking: ROW_NUMBER() OVER() replaces MySQL @a:=@a+1; offline filter injected dynamically
	private static final String UPDATE_RANKING_LIST_PLAYERS_POSITIONS_BASE = "UPDATE abyss_rank SET rank_pos = sub.rn FROM ("
		+ "SELECT player_id, ROW_NUMBER() OVER (ORDER BY gp DESC) AS rn FROM abyss_rank WHERE gp > 0 AND player_id IN ("
		+ "SELECT id FROM players WHERE race = ?%s) LIMIT ?) sub WHERE abyss_rank.player_id = sub.player_id";
	// Legion ranking: ROW_NUMBER() OVER() replaces MySQL @a:=@a+1
	private static final String UPDATE_RANKING_LIST_LEGIONS_POSITIONS = "UPDATE legions SET rank_pos = sub.rn FROM ("
		+ "SELECT l.id, ROW_NUMBER() OVER (ORDER BY l.contribution_points DESC) AS rn FROM legions l WHERE l.id IN ("
		+ "SELECT legion_id FROM legion_members lm JOIN players p ON p.id = lm.player_id WHERE lm.rank = 'BRIGADE_GENERAL' AND p.race = ?) LIMIT ?"
		+ ") sub WHERE legions.id = sub.id";
	...
	public static List<RankingListPlayer> loadRankingListPlayers() {
		...
		try (Connection con = DatabaseFactory.getConnection();
			...
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
				int position = rs.getInt("rank_pos");
				int oldPosition = rs.getInt("old_rank_pos");
				int id = rs.getInt("id");
				String name = rs.getString("name");
				Race race = Race.valueOf(rs.getString("race"));
				int level = DataManager.PLAYER_EXPERIENCE_TABLE.getLevelForExp(rs.getLong("exp"));
				int rank = rs.getInt("rank");
				int ap = rs.getInt("ap");
				int gp = rs.getInt("gp");
				int title = rs.getInt("title_id");
				PlayerClass playerClass = PlayerClass.valueOf(rs.getString("player_class"));
				Gender gender = Gender.valueOf(rs.getString("gender"));
				String legionName = rs.getString("legion_name");
	...
	public static List<RankingListLegion> loadRankingListLegions() {
		...
		try (Connection con = DatabaseFactory.getConnection();
			...
			while (rs.next()) {
				...
				long contributionPoints = rs.getLong("contribution_points");
	...
	public static void updateRankingLists(int maxOfflineDays, int playerLimit, int legionLimit) {
		// Build the player ranking SQL with optional offline filter
		String offlineFilter = maxOfflineDays > 0
			? " AND last_online >= CURRENT_DATE - (" + maxOfflineDays + " || ' days')::INTERVAL"
			: "";
		String playerRankingSql = String.format(UPDATE_RANKING_LIST_PLAYERS_POSITIONS_BASE, offlineFilter);
		...
		try (Connection con = DatabaseFactory.getConnection()) {
			...
			// Update player rankings for each race using ROW_NUMBER()
			try (PreparedStatement stmt = con.prepareStatement(playerRankingSql)) {
			...
			// Reset legion rankings
			try (PreparedStatement stmt = con.prepareStatement(RESET_RANKING_LIST_LEGIONS)) {
				stmt.executeUpdate();
			}
			// Update legion rankings for each race using ROW_NUMBER()
