	...
	public void connect(Executor dcExecutor) {
		try {
			...
			for (ServerCfg cfg : cfgs) {
				...
				// expectProxyV2 flows from ServerCfg so each port on this NioServer can opt in
				// independently (typical: external game port on, internal GS-to-LS off).
				SelectionKey acceptKey = getAcceptDispatcher().register(serverChannel, SelectionKey.OP_ACCEPT,
					new Acceptor(cfg.connectionFactory(), this, cfg.expectProxyV2()));
