...
import com.aionemu.loginserver.utils.TokenHandoff;
...
public class AccountController {
	...
	/**
	 * Authenticates a player using a ShiguangCloud session token.
	 * <p>
	 * The token (password without "SG-" prefix) is validated against the
	 * Control server via HTTP. On success, the account is loaded or auto-created
	 * in the local database. All post-login checks (ban, expiry, double login)
	 * are applied identically to normal password login.
	 * <p>
	 * This method is independent of ExternalAuth and uses a separate HTTP
	 * endpoint (Config.TOKEN_HANDOFF_URL).
	 *
	 * @param name       username from CM_LOGIN (may be overridden by token result)
	 * @param password   "SG-{token}" — the full password field
	 * @param connection the client connection
	 * @return auth response, or null if account is banned
	 */
	public static AionAuthResponse loginWithToken(String name, String password, LoginConnection connection) {
		// Check IP ban first (same as normal login)
		if (BannedIpController.isBanned(connection.getIP())) {
			return AionAuthResponse.STR_L2AUTH_S_BLOCKED_IP;
		}

		// Extract and validate the token
		String token = TokenHandoff.extractToken(password);
		TokenHandoff.Result result = TokenHandoff.validate(token, connection.getIP());

		if (result == null) {
			// Control server unreachable or misconfigured
			return AionAuthResponse.STR_L2AUTH_S_ACCOUNTCACHESERVER_DOWN;
		}
		if (!result.ok() || result.account() == null || result.account().isEmpty()) {
			// Token invalid, expired, or already consumed
			return AionAuthResponse.STR_L2AUTH_S_INCORRECT_PWD;
		}

		// Token validated — use the account name from the token
		String accountName = result.account();

		// Load or auto-create account
		Account account = loadAccount(accountName);
		if (account == null && Config.ACCOUNT_AUTO_CREATION) {
			account = createAccount(accountName, ""); // no password hash needed for token auth
		}
		if (account == null) {
			return AionAuthResponse.STR_L2AUTH_S_ACCOUNT_LOAD_FAIL;
		}

		// Skip password verification — already validated via token

		// Apply all standard post-login checks (identical to normal login)
		if (account.getActivated() != 1) {
			return AionAuthResponse.STR_L2AUTH_S_AGREE_GAME;
		}
		if (AccountTimeController.isAccountExpired(account)) {
