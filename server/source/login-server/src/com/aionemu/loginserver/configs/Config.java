...
public class Config {
	...
	/**
	 * When true, expect HAProxy PROXY Protocol v2 header from shiguang-gate on
	 * every Aion client connection. Only the client port is affected; the
	 * internal GS-to-LS port always stays direct.
	 */
	@Property(key = "loginserver.network.client.expect_proxy_v2", defaultValue = "false")
	public static boolean CLIENT_EXPECT_PROXY_V2;
	...
	/**
	 * URL for ShiguangCloud Token Handoff validation endpoint.
	 * Example: http://127.0.0.1:10443/api/token/validate
	 * When set, CM_LOGIN passwords starting with "SG-" are validated
	 * as session tokens against this endpoint instead of the local DB.
	 * This works independently of (and alongside) external auth.
	 */
	@Property(key = "loginserver.shiguang.token_handoff.url", defaultValue = "")
	public static String TOKEN_HANDOFF_URL;

	/**
	 * Whether Token Handoff is configured and should be checked.
	 */
	public static boolean useTokenHandoff() {
		return TOKEN_HANDOFF_URL != null && !TOKEN_HANDOFF_URL.isBlank();
	}
