...
public class CM_PLAY extends AionClientPacket {
	...
	private static final Logger log = LoggerFactory.getLogger(CM_PLAY.class);
	...
	protected void runImpl() {
		...
		log.info("[LOG-DEBUG] CM_PLAY received: accountId={}, loginOk={}, servId={}", accountId, loginOk, servId);
		...
		if (key.checkLogin(accountId, loginOk)) {
			...
			if (gsi == null || !gsi.isOnline())
				...
				con.setJoinedGs();
				log.info("[LOG-DEBUG] Sending SM_PLAY_OK for account {} to server {}", accountId, servId);
					sendPacket(new SM_PLAY_OK(key, servId));
			}
		} else {
			log.warn("[LOG-DEBUG] CM_PLAY auth FAILED for accountId={}", accountId);
