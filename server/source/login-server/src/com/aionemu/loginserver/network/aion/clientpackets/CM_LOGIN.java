...
public class CM_LOGIN extends AionClientPacket {
	...
	protected void runImpl() {
		...
		// Token Handoff: if password starts with "SG-", validate the session
		// token against ShiguangCloud Control instead of local DB password.
		// Independent of ExternalAuth — both can coexist.
		AionAuthResponse response;
		if (Config.useTokenHandoff() && TokenHandoff.isTokenHandoff(loginData.password)) {
			response = AccountController.loginWithToken(loginData.username, loginData.password, client);
		} else {
			response = AccountController.login(loginData.username, loginData.password, client);
		}
