...
public class BannedMacDAO {
	...
	public static boolean update(BannedMacEntry entry) {
		try (Connection con = DatabaseFactory.getConnection();
			PreparedStatement ps = con.prepareStatement("INSERT INTO banned_mac (address,time,details) VALUES (?,?,?) ON CONFLICT (address) DO UPDATE SET time = EXCLUDED.time, details = EXCLUDED.details")) {
