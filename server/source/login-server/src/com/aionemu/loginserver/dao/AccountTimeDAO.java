...
public class AccountTimeDAO {
	...
	public static boolean updateAccountTime(int accountId, AccountTime accountTime) {
		return DB.insertUpdate(
			"INSERT INTO account_time (account_id, last_active, expiration_time, session_duration, accumulated_online, accumulated_rest, penalty_end) VALUES "
				+ "(?,?,?,?,?,?,?) ON CONFLICT (account_id) DO UPDATE SET last_active=EXCLUDED.last_active, expiration_time=EXCLUDED.expiration_time, "
				+ "session_duration=EXCLUDED.session_duration, accumulated_online=EXCLUDED.accumulated_online, "
				+ "accumulated_rest=EXCLUDED.accumulated_rest, penalty_end=EXCLUDED.penalty_end",
			ps -> {
