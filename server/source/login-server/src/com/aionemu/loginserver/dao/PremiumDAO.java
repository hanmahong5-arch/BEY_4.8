...
public class PremiumDAO {
	...
	public static long getPoints(int accountId) {
		...
		try (Connection con = DatabaseFactory.getConnection();
			PreparedStatement st = con.prepareStatement("SELECT uniqId,points FROM account_rewards WHERE accountId=? AND rewarded=false")) {
			st.setInt(1, accountId);
			try (ResultSet rs = st.executeQuery()) {
				if (rs.next()) {
		...
		if (rewarded.size() > 0) {
			try (Connection con = DatabaseFactory.getConnection();
				PreparedStatement stmt = con.prepareStatement("UPDATE account_rewards SET rewarded=true,received=NOW() WHERE uniqId=?")) {
