...
public class AccountDAO {
	...
	private static final String ACCOUNT_NAME_COLUMN = Config.useExternalAuth() ? "ext_auth_name" : "name";

	public static Account getAccount(String name) {
		return getAccount("SELECT * FROM account_data WHERE " + ACCOUNT_NAME_COLUMN + " = ?", name);
	}
		...
		try (Connection con = DatabaseFactory.getConnection(); PreparedStatement st = con.prepareStatement(accountQuery)) {
			...
			try (ResultSet rs = st.executeQuery()) {
				if (rs.next()) {
					...
					account.setActivated((byte) (rs.getBoolean("activated") ? 1 : 0));
	...
	public static boolean insertAccount(Account account) {
		try (Connection con = DatabaseFactory.getConnection();
			PreparedStatement st = con.prepareStatement("INSERT INTO account_data(" + ACCOUNT_NAME_COLUMN
			...
			st.setBoolean(5, account.getActivated() != 0);
	...
	public static boolean updateAccount(Account account) {
		try (Connection con = DatabaseFactory.getConnection();
			PreparedStatement st = con.prepareStatement("UPDATE account_data SET " + ACCOUNT_NAME_COLUMN
